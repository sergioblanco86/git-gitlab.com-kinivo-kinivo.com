		   ...
		   if(!isset($flatsome_opt['breadcrumb_home']) || $flatsome_opt['breadcrumb_home']){ 
			...
			if ( $terms = wp_get_post_terms( $post->ID, 'product_cat', array( 'orderby' => 'parent', 'order' => 'DESC' ) ) ) {
				...
				foreach ( $ancestors as $ancestor ) {
					...
					echo $before . '<a href="' . get_term_link( $ancestor->slug, 'product_cat' ) . '">' . ucfirst($ancestor->name) . '</a>' . $after . $delimiter;
				}

				echo $before . '<a href="' . get_term_link( $main_term->slug, 'product_cat' ) . '">' . ucfirst($main_term->name) . '</a>' . $after;
			}
