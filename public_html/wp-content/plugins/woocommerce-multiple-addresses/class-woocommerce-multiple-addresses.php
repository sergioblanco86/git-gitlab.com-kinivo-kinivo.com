...
class WC_Multiple_addresses {
	...
	public function multiple_shipping_addresses() {
		...
		if ( ! empty( $otherAddr ) ) {
			...
			foreach ( $otherAddr as $idx => $address ) {
				...
				echo '<p align="right"><a href="#" class="delete">' . __( '', $this->plugin_slug ) . '</a></p>';
		...
		echo '<div class="form-row">
                ...
                <input type="submit" name="set_addresses" value="' . __( 'Save Addresses', $this->plugin_slug ) . '" class="button alt submit-button standar-nowidth green" />
	...
	public function before_checkout_form() {
	  ...
	  //               <a href="' . get_permalink( $page_id ) . '">' . __( 'Configure Address', $this->plugin_slug ) . '</a>
	  //             </p>';
		}
	}
	...
	public function add_dd_to_checkout_fields( $fields ) {
		...
		$alt_field = array(
			'label'    => __( 'Saved Addresses', $this->plugin_slug ),
			'required' => false,
