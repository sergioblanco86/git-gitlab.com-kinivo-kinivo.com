...
class TwitterStrategy extends OpauthStrategy {
	...
	public function request() {
		...
		if ( $results !== false && !empty( $results['oauth_token'] ) && !empty( $results['oauth_token_secret'])  ){
			...
			set_transient( 'wcsl_' . $results['oauth_token'], $results, HOUR_IN_SECONDS );
	...
	public function oauth_callback() {
		...
		if ( ! empty( $_REQUEST['oauth_token'] ) && $_REQUEST['oauth_token'] && $session = get_transient( 'wcsl_' . $_REQUEST['oauth_token'] ) ) {

			delete_transient( 'wcsl_' . $_REQUEST['oauth_token'] );
			...
			if ($results !== false && !empty($results['oauth_token']) && !empty($results['oauth_token_secret'])) {
				...
				if (!empty($credentials['id'])) {
					...
					$credentials['profile_image_url_https'] = str_replace( '_normal.', '.', $credentials['profile_image_url_https'] );
