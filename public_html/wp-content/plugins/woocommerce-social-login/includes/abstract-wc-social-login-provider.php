...
abstract class WC_Social_Login_Provider extends WC_Settings_API {
	...
	public function create_new_customer( $profile ) {
		...
		$userdata = apply_filters( 'wc_social_login_' . $this->id . '_new_user_data', array(
			'role'       => 'customer',
			'user_login' => $profile->has_email() ?  substr(sanitize_email( $profile->get_email() ), 0, strpos(sanitize_email( $profile->get_email() ), '@'))  : $profile->get_nickname(),
	...
	public function get_callback_url() {
		...
		$force_ssl = $this->requires_ssl() || ( apply_filters( 'wc_social_login_force_ssl_callback', false, $this ) );
